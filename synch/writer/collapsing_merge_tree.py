...
class ClickHouseCollapsingMergeTree(ClickHouseMergeTree):
    ...
    def get_table_create_sql(
        ...
        return f"CREATE TABLE {schema}.{table}{cluster_sql(self.cluster_name)}ENGINE = {self.engine}({sign_column}) {partition_by_str} ORDER BY {order_by} {engine_settings_str} AS {select_sql} limit 0"
