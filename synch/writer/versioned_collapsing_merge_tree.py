...
class ClickHouseVersionedCollapsingMergeTree(ClickHouseCollapsingMergeTree):
    ...
    def get_table_create_sql(
        ...
        super(ClickHouseVersionedCollapsingMergeTree, self).get_table_create_sql(
            reader, schema, table, pk, partition_by, engine_settings, order_by, sign_column=sign_column
        )
        ...
        return f"CREATE TABLE {schema}.{table}{cluster_sql(self.cluster_name)} ENGINE = {self.engine}({sign_column},{version_column}) {partition_by_str} ORDER BY {order_by} {engine_settings_str} AS {select_sql} limit 0"
